...
export function Typography({ variant, component, className, ...props }: TypographyProps) {
  const Comp = component ?? variant ?? 'h1';

  return <Comp className={cn(typographyVariants({ variant, className }), className)} {...props} />;
}
