...
import { createPaymentCheckoutSession, type Plan, type Stripe } from '@/adapters/payment';
...
const renderProduct = (product: string | Stripe.Product | Stripe.DeletedProduct) => {
...
export function CheckoutForm({ plans }: CheckoutFormProps) {
  const handleSubmit = async (priceId: string) => {
    const secureStripe = loadStripe(process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY ?? '');
